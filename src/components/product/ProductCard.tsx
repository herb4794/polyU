...
const ProductCard = ({ porduct_description, product_discount, product_en_name, product_image, setOrderArr }: any) => {

  const [order, setOrder] = useState<any>({ product_en_name: product_en_name, product_discount: product_discount, quantity: 1, product_image: product_image })

  const productName = product_en_name && product_en_name.length > 15 ? product_en_name.substring(0, 25) + '...' : product_en_name;
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
      <div className="h-56 w-full">
        <div >
          <img className="mx-auto h-full dark:hidden" src={product_image} alt="" />
        </div>
      </div>
      <div className="pt-6">
        ...
        <div className="text-lg font-semibold leading-tight text-gray-900 hover:underline dark:text-white">{productName}</div>
        ...
        <div className="mt-4 flex items-center justify-between gap-4">
          <p className="text-2xl font-extrabold leading-tight text-gray-900 dark:text-white">${product_discount}</p>
